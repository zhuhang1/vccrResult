...
parser.add_argument('--vtcp',
                    dest="vtcp",
                    help="vtcp value for the first sender",
                    default="0")
...
def set_red(iface, red_params):
    ...
    cmd = ("tc qdisc change dev %s parent 5:1 handle 6: "
	   ...
	   "burst %s bandwidth 10Mbit probability %s ecn" % (iface, red_params['limit'], red_params['min'], red_params['max'], red_params['avpkt'], red_params['burst'], red_params['prob']))
    print cmd
    os.system(cmd)
...
def tcpecn():
    ...
    topo = StarTopo(n=args.hosts, bw_host=args.bw_host, 
		    ...
		    bw_net=args.bw_net,
		    maxq=args.maxq,
		    enable_ecn= 1 if args.ecn==1 or args.vtcp==1 else 0,
		    enable_red=args.red,
    ...
    os.system("sudo dumpcap -i s0-eth2 -a duration:37 &")
    ...
    start_receiver(net)
    start_senders(net,args.ecn,args.cong,args.vtcp)
